{{ config(
    alias = 'snowflake_query_usage_1d',
...
SELECT 
    ...
    -- Time dimensions (only keep query_hour since others come from date dimension)
    q.QUERY_HOUR,
    
    -- Environment classification for cost allocation
    CASE 
        WHEN q.WAREHOUSE_ENVIRONMENT = 'PRD' THEN q.ESTIMATED_QUERY_COST_USD
        ELSE 0 
    END AS production_cost_usd,
    
    CASE 
        WHEN q.WAREHOUSE_ENVIRONMENT IN ('DEV', 'TEST') THEN q.ESTIMATED_QUERY_COST_USD
        ELSE 0 
    END AS development_cost_usd,
    
    CASE 
        WHEN q.WAREHOUSE_ENVIRONMENT = 'STG' THEN q.ESTIMATED_QUERY_COST_USD
        ELSE 0 
